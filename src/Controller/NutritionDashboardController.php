...
class NutritionDashboardController extends AbstractController
{
    ...
    public function index(Request $request, RepasRepository $repasRepository, \App\Repository\AlerteRepository $alerteRepository, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($userId && $this->isGranted('ROLE_ADMIN')) {
            $user = $entityManager->getRepository(\App\Entity\Utilisateur::class)->find($userId);
            if (!$user) {
        ...
        // Debug: Log pour voir les repas
        $debugInfo = [
            'user_id' => $user->getId(),
            'startDate' => $startDate->format('Y-m-d H:i:s'),
            'allRepas_count' => count($allRepas),
        ];
        
        $filteredRepas = array_filter($allRepas, function($repas) use ($startDate) {
            $dateConso = $repas->getDateConsommation();
            return $dateConso >= $startDate;
        });
        
        $debugInfo['filtered_count'] = count($filteredRepas);
        error_log('NutritionDashboard Debug: ' . json_encode($debugInfo));
