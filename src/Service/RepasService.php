...
class RepasService
{
    ...
    public function calculerScoreIntelligent(Repas $repas): int
    {
        ...
        // 3. BONUS : Si l'horaire est bon, on donne toujours +4 points minimum
        $timingScore = $this->calculerScoreTiming($repas);
        if ($timingScore > 0) {
            $score = max($score, 4); // Minimum 4 points si horaire correct
        }
    ...
    private function calculerScoreTiming(Repas $repas): int
    {
        ...
        return match($typeMoment) {
            TypeMoment::MATIN => ($heure >= 6 && $heure <= 10) ? 5 : -3,
            TypeMoment::MIDI => ($heure >= 11 && $heure <= 14) ? 4 : -2,
            TypeMoment::COLLATION => ($heure >= 9 && $heure <= 11 || $heure >= 15 && $heure <= 17) ? 2 : -1,
            TypeMoment::SOIR => ($heure >= 18 && $heure <= 21) ? 3 : -2,
        };
    }
    ...
    {
        return [
            'Matin (6h-10h)' => '+5 timing',
            'Midi (11h-14h)' => '+4 timing',
            'Collation (9h-11h / 15h-17h)' => '+2 timing',
            'Soir (18h-21h)' => '+3 timing',
        ];
    }
