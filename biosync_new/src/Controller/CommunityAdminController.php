...
class CommunityAdminController extends AbstractController
{
    ...
    public function index(Request $request, GroupeSoutienRepository $groupRepo, EvenementSanteRepository $eventRepo): Response
    {
        ...
        if ($search) {
            ...
            $orderBy = match($sort) {
                'name' => ['titreEvent' => 'ASC'],
                'oldest' => ['dateEvent' => 'ASC'],
                default => ['dateEvent' => 'DESC'],
            };
            $events = $eventRepo->findBy([], $orderBy);
        }
    ...
    // ... (Gardez les méthodes newGroup, editGroup, deleteGroup, newEvent, editEvent, deleteEvent sans changement)
